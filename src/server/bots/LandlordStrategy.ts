...
export class LandlordStrategy extends RandoBotStrategy implements BotStrategy {
  ...
  public override selectCorporation(
    ...
    return (
      cards.find((card) => card.name === CardName.CREDICOR) ??
  ...
  public override selectInput(
    ...
    if (input.title === 'Select space for city tile') {
      return this.selectPreferredSpace(input.spaces, random, (space) =>
        this.commonPlacementScore(
          player,
          space,
          player.game.board
            .getAdjacentSpaces(space)
            .filter(Board.isGreenerySpace).length,
        ),
      );
    }
    if (input.title === 'Select space for greenery tile') {
      return this.selectPreferredSpace(input.spaces, random, (space) => {
        const adjacentCities = player.game.board
          .getAdjacentSpaces(space)
          .filter(Board.isCitySpace);
        const ownedCities = adjacentCities.filter(
          (adjacent) => adjacent.player === player,
        ).length;
        const opponentCities = adjacentCities.length - ownedCities;
        return this.commonPlacementScore(
          player,
          space,
          ownedCities,
          -opponentCities,
  ...
  private tryGreeneryProject(player: IPlayer): boolean {
    ...
    const hasGreenerySpaceByCity = player.game.board.spaces
      ...
      .some((city) =>
        player.game.board
          .getAdjacentSpaces(city)
          .some((space) => available.includes(space)),
      );
    return (
      hasGreenerySpaceByCity &&
      this.tryStandardProject(player, CardName.GREENERY_STANDARD_PROJECT)
    );
  }

  private tryStandardProject(player: IPlayer, name: CardName): boolean {
    const project = player.game
      .getStandardProjects()
  ...
  private commonPlacementScore(
    ...
    const adjacentOceans = player.game.board
      .getAdjacentSpaces(space)
      .filter(Board.isOceanSpace).length;
