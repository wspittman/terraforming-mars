...
export class RandoBotStrategy implements BotStrategy {
  ...
  public selectCorporation(
    cards: ReadonlyArray<ICorporationCard>,
    random: Random,
  ): ICorporationCard | undefined {
  ...
  public selectCard(input: SelectCard<ICard>, random: Random): Array<CardName> {
    ...
    const card = selectRandomElement(
      input.cards.filter((_, index) => input.config.enabled?.[index] !== false),
      random,
    );
  ...
  public selectInput(
    ...
    if (input instanceof SelectInitialCards) {
      const corporation = this.selectCorporation(
        input.player.dealtCorporationCards,
        random,
      );
    ...
    throw new Error(
      `Bot strategy ${this.name} cannot resolve ${input.type} input`,
    );
  }
}
