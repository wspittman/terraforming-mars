...
export class ChooseCards extends DeferredAction {
  ...
  public execute() {
    ...
    let msg: string | Message = message('Select ${0} card(s) to keep', (b) =>
      b.number(max),
    );
    if (options.paying) {
      const spendableMegacredits = this.player.spendableMegacredits();
      const affordableCards = Math.floor(
        spendableMegacredits / this.player.cardCost,
      );
    ...
    return new SelectCard(msg, button, cards, {
      max,
      min,
      played: !options.paying,
    }).andThen((selected) => {
      ...
      const boughtLogType =
        options.logBoughtCards === true ?
          LogType.BOUGHT_VERBOSE :
          LogType.BOUGHT;
      if (options.paying && selected.length > 0) {
        const cost = selected.length * player.cardCost;
        player.game.defer(
          new SelectPaymentDeferred(player, cost, {
            title: message('Select how to spend ${0} M€ for ${1} cards', (b) =>
              b.number(cost).number(selected.length),
            ),
        ...
        keep(
          ...
          selected,
          unselected,
          options.paying ? boughtLogType : LogType.DREW,
        );
      }
      return undefined;
    });
...
export function keep(
  ...
  case LogType.BOUGHT_VERBOSE:
    if (cards.length === 0) {
      ...
      player.game.log('${0} bought ${1}', (b) =>
        b.player(player).cards(cards),
      );
    }
    break;
  case LogType.DREW:
  case LogType.BOUGHT:
    player.game.log('${0} ${1} ${2} card(s)', (b) =>
      b.player(player).string(logType).number(cards.length),
    );
    if (logType === LogType.BOUGHT) {
      if (cards.length > 0) {
        player.game.log('You bought ${0}', (b) => b.cards(cards), {
          reservedFor: player,
        });
      }
    } else {
