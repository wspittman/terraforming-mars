...
describe('Game', () => {
  ...
  it('Should return players in turn order', () => {
    ...
    expect(game.playersInGenerationOrder.map(toName)).deep.eq([
      'p3',
      'p4',
      'p1',
      'p2',
  ...
  it('Milestones can be claimed', () => {
    ...
    expect(
      actions.options.find((option) => option.title === 'Claim a milestone'),
    ).is.undefined;

    player.megaCredits = 8;
    const actions2 = cast(player.getActions(), OrOptions);
    const claimMilestoneAction = cast(
      actions2.options.find((option) => option.title === 'Claim a milestone'),
      OrOptions,
    );
    claimMilestoneAction.options[0].cb();
    runAllActions(game);
    expect(
      game.claimedMilestones.some(
        (cm) => cm.milestone.name === 'Terraformer' && cm.player === player,
      ),
    ).is.true;
  });
  ...
  it('Milestones cannot be claimed twice', () => {
    ...
    expect(() => claimMilestoneAction.options[0].cb()).to.throw(
      /Terraformer is already claimed/,
    );
    const actions2 = cast(player.getActions(), OrOptions);
    expect(
  ...
  it('fails when first player is absent from the list of players.', () => {
    expect(() =>
      Game.newInstance(
        'gameid',
        [TestPlayer.RED.newPlayer(), TestPlayer.BLUE.newPlayer()],
        TestPlayer.YELLOW.newPlayer(),
        'spectatorid',
      ),
