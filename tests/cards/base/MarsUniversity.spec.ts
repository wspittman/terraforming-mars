...
describe('MarsUniversity', () => {
  ...
  it('verifying Mars U play order', () => {
    ...
    const olympusConferenceAction = cast(
    ...
    const marsUniversityAction = cast(
      game.deferredActions.pop()?.execute(),
      OrOptions,
    );
    expect(marsUniversityAction.options[0].title).to.match(
      /Select a card to discard/,
    );
    // Second option does nothing.
    marsUniversityAction?.options?.[1].cb();

    const roboticWorkforceAction = cast(
      game.deferredActions.pop()?.execute(),
      SelectCard,
    );
